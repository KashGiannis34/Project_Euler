package Problems;//Maximum path sum I
...
public class Problem_18 {
    public static ArrayList<ArrayList<Integer>> toListList(String[] array) {
        ...
        for (int n = 1; n < array.length; n++) {
            temp = new ArrayList<>();
            String[] splitnums = array[n].split(" ");
    ...
    public static void main(String[] args) {
        ArrayList<ArrayList<Integer>> pyramid = toListList(dalist());
        for (int n = pyramid.size() - 2; n >= 0; n--) {
            for (int x = 0; x <= n; x++) {
                pyramid.get(n).set(x, pyramid.get(n).get(x)+ Math.max(pyramid.get(n+1).get(x), pyramid.get(n+1).get(x+1)));
            }
        }
